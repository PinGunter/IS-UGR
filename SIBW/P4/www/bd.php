<?php
    ...
    function editarProducto($datos, $imgs){
        ...
        $nuevosDatos = "UPDATE productos set nombre=?, descripcion=?, precio=?, id_marca=? WHERE id=?";
        if (!($stmt = $mysqli->prepare($nuevosDatos))) {
            echo "Falló la preparación: (" . $mysqli->errno . ") " . $mysqli->error;
            return false;
        }
        if (!$stmt->bind_param("ssdii", $datos["nombre"], $datos["descripcion"], $datos["precio"], $datos["id_marca"], $datos["pid"])){
            echo "Falló la vinculación de parámetros: (" . $stmt->errno . ") " . $stmt->error;
            return false;
        }
        if (!$stmt->execute()) {
